...
One file per **phase** (`vA.B`). IDs (`H11-###`) are **globally sequential** and
...
5. **Find the next free `H11-###` id — never restart the numbering.** Ids are globally
   ...
   The next id is **`max(a, b) + 1`**, zero-padded to three digits (`H11-001`, `H11-047`,
   `H11-118`). Start at `H11-001` **only if both sources come back empty**.
...
| 2 | H11-{…} | {title} | S | {area} | v{A.B} | H11-{first} |
| … | … | … | … | … | … | … |
...
{repeat the `### H11-{id} …` block per issue}

## v{A.B} scope notes

**Total effort:** {rough estimate}.
**Critical path:** H11-{…} → … → H11-{…}.
...
- Generated on upload: `v{A.B}-github-report.md` (H11-### → GitHub #), then `v{A.B}-execution-report.md`.
````
...
phase=vA,version=vA.B --status ok --data '{"issues":[{"id":"H11-001","size":"M"},...]}'`.
...
Show the user: the file path, the issue count, the `H11-###` id range, and the
...
- **IDs are globally sequential** (`H11-###`), continuing across phase files **and across
  regeneration runs** — never reset. Resolve the next id from `max(GitHub, local issues files) + 1`
  (Step 0.5); starting at `H11-001` is correct only when both sources are genuinely empty.
