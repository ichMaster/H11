...
6. Confirm the working dev branch + a clean tree; establish a **green baseline** (`--import` clean + `--smoke` green) and **record the baseline test count** (the "before" for statistics). Never start red.
...
2. **`execute-issues-file vA.B`** — implement each issue from the reconciled file in dependency order
   → validate (`--import` clean + `--smoke` green, **LLM mocked**) → commit (one issue = one commit) → push;
...
- Suite: <baseline> → <final> tests passing · check_palette.py clean · zero paid calls
...
- **Sequential and gated; stop on failure.** Any sub-skill failure or a red `--smoke`/`check_palette.py` halts the
