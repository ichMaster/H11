...
1. Resolve the target to `specification/implementation/vA.B-issues.md` and read it fully (summary table,
   dependency tree, each `### H11-### …` section).
...
- Wrong or renamed **file/module** paths (e.g. `scripts/entity.gd` vs the real `scripts/enemy.gd`).
...
2. **Add a visible change-mark** as a blockquote directly under the issue heading, e.g.:
   ...
   > the shipped `scripts/enemy.gd` reading its tuning from `data/entities.json`. Reason: matches the real
   > implementation.
