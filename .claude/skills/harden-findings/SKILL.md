...
1. Confirm the working dev branch and a clean tree; establish a **green baseline** (`--import` clean + `--smoke` green). Never harden on a red suite — fix a clear flake first (own commit) or surface it.
...
1. Re-run the **full suite** once after the sweep — green and deterministic — plus `check_palette.py`.
...
fix → `harden.finding.fixed` (`finding`, `sha`, **plus `--smoke` with the full-suite counts after the
...
- **Stop on failure.** A red `--smoke`/`check_palette.py` on any fix halts the sweep — report what landed and what
