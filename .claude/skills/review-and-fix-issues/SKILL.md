...
3. **Establish a green baseline** — run `--import` and `--smoke`. If the suite is **red or flaky**,
...
- **Input hygiene & secrets:** unvalidated deck/table data reaching the builder; the device password reaching or being logged
...
3. **Validate:** `--smoke` green and `check_palette.py` clean. Only commit code that passes.
...
- Add a **"Fixes applied"** section: per fix, the change, the regression test, and the verification
  (final `--smoke` and `check_palette.py` status). For any fix that **changed a documented contract or a
...
`finding.fixed` (`sha`, **plus `--smoke` with the full-suite counts after the fix**) for each fix-now
...
homes); the final green `--smoke` and `check_palette.py` status. If fixes landed on an already-released phase,
...
- **Green before, green after.** Establish a green baseline; only commit code that passes `--smoke` +
  `check_palette.py`; keep the gates deterministic.
