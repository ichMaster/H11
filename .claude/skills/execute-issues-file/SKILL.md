...
```
/execute-issues-file <vA.B | path-to-issues-file> [--issue H11-###] [--dry-run]
```
...
- `--issue H11-###` → only that issue (its file-listed deps must already be committed)
...
4. Establish a **green baseline** (`--import` clean + `--smoke` green) so a later failure is attributable.

### Step 1: Build the execution queue (from the file)

- Parse the H11-### IDs + titles from the file's summary table; order them by the file's
...
- **Skip issues already implemented** — an issue whose ID already appears in a prior commit
  (`git log --grep "H11-###:"`) is done; skip it (resumability).
- With `--issue H11-###`, execute only that one (verify its file-listed deps are already committed).
...
3. **Implement** per `CLAUDE.md` + `specification/ARCHITECTURE.md`, routed by component
   ([ARCHITECTURE.md](../../../specification/ARCHITECTURE.md)): `scripts/game.gd` (the only autoload;
   signals are the only cross-system channel), `scripts/level.gd` (the only place that knows the map
   language), the entity scripts (duck-typed `interact`/`hit`), `shaders/` (no lights; clockwise
   winding), `tools/` (assets are committed output — the source is the generator). A **seam change**
   (the map language, `interact`/`hit`, the `Game` signals, collision layers, a `data/` table shape, the WS
   event/action shapes, seat-by-token identity) updates `specification/ARCHITECTURE.md` **and** its contract
   test in the **same** commit. Strict typing; MVP-first (don't pull later phases in early).
4. **Validate:** `--import`, `--smoke`, and `check_palette.py` / `--drive` when the change warrants them — **the
   engine runs headless and nothing calls a paid API.** Walk each acceptance criterion against
...
- **No broken code.** Only commit what passes `--import` clean + `--smoke` green.
