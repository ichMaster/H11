...
6. Confirm we are on the working dev branch and the tree is clean; establish a **green baseline**
   (`--import` clean + `--smoke` green). Never start on a red suite — fix a clear flake first or surface it.
...
- **Stop on failure — do not paper over it.** If any sub-skill fails, or any fix hits a red
  `--smoke`/`check_palette.py`, halt, report what completed and what remains, and let the user decide. Never release
...
- **Delegate, never duplicate.** This skill only sequences the sub-skills (`generate-issues`,
  ...
  `H11-###` namespace, releases use unprefixed `vA.B.C` tags, every line generated fresh.
