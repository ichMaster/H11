...
`H11-###` issues by `/generate-issues`. If the file does not exist yet, run
...
- `Dependencies` (list of H11-### IDs)

Then parse each **detailed issue section** (heading with H11-###) to extract:
...
3. Record the mapping: H11-### -> GitHub issue #number
4. Report to user: `Created H11-### -> #{number}: {title}`
5. If the issue depends on already-created issues, add a comment:
   ```bash
   gh issue comment {issue-number} --body "Blocked by #{dep-issue-number} (H11-###)"
   ```
...
|----------|----------|-------|-------|--------|-----|
| H11-001 | #5 | ... | v1.2 | v1.2::phase, v1.2::size:S, v1.2::area:games | {url} |
...
(`--emitter skill:upload-issues --scope phase=..,version=..,step=upload-issues,issue=H11-###`).
