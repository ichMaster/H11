...
- **[specification/ROADMAP.md](specification/ROADMAP.md)** — eight versions, each phase with Goal,
  Tasks, DoD and the checks that encode the DoD. Current state: **`0.10.0` released** (music and the shotgun; prototype hardened,
  the review's High and Medium findings closed). **v0.9 — input as data** is four sixths done and not yet released; two issues need hands on the device.
...
**Input is data** (v0.9): [data/input.json](data/input.json) holds the actions once and binds keys to them per profile — `desktop` for the development keyboard, `device` for the PocketTerm, chosen by `Game.on_device`. Two profiles because the device's buttons send the same physical keycodes the desktop scheme uses. Physical keycodes throughout, so the scheme survives a keyboard-language change. The table is validated at load (every profile, not just the active one) and `tools/check_input_fixtures.sh` is the gate. Adding an action is a line in that file, never GDScript. See [ARCHITECTURE.md](specification/ARCHITECTURE.md) §Input.

**Lighting is faked in a shader.** There are no lights at all: [depth_shade.gdshader](shaders/depth_shade.gdshader) is `unshaded` with exponential distance darkening, because `Environment` fog is unreliable on the Compatibility renderer (see the comment in [main.gd](scripts/main.gd) `_setup_environment()`). Two things must stay in sync: walls/floors use the shader uniform, and billboard sprites (enemies, pickups) call `Level.depth_shade(pos)` which recomputes the *same* exponential in GDScript. `Level.FOG_DENSITY` is the single knob, currently 0.031 (the art was authored against 0.085; lower = see further) — change it and **both** shader defaults together. Emergency-light flicker is `main.gd` `_process()` poking `tint` on the shared `wall_light` material.
